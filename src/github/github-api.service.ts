...
export class GithubApiService {
  ...
  async getAuthenticatedUserLogin(accessToken: string): Promise<string> {
    const res = await fetch(`${GITHUB_API}/user`, { headers: this.headers(accessToken) });
    ...
    if (!body.login) throw new BadGatewayException('Invalid GitHub /user response');
    return body.login;
  }
  ...
  async listUserRepos(accessToken: string): Promise<GithubRepoListItem[]> {
    ...
    for (;;) {
      ...
      const res = await fetch(url.toString(), { headers: this.headers(accessToken) });
      if (res.status === 401) throw new UnauthorizedException('GitHub token rejected');
      if (!res.ok) {
        throw new BadGatewayException(`GitHub /user/repos failed: ${res.status}`);
      }
  ...
  async getRepo(accessToken: string, owner: string, repo: string): Promise<GithubRepoDetail> {
    const res = await fetch(`${GITHUB_API}/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}`, {
      headers: this.headers(accessToken),
    });
    if (res.status === 401) throw new UnauthorizedException('GitHub token rejected');
    if (res.status === 404) {
      throw new ForbiddenException('Repository not found or not accessible with this token');
    }
    if (!res.ok) {
  ...
  async assertUserOwnsRepo(accessToken: string, fullName: string): Promise<GithubRepoDetail> {
    ...
    if (owner.toLowerCase() !== login.toLowerCase()) {
      throw new ForbiddenException('You can only scan repositories you own on your personal account');
    }
