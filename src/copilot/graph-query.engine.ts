...
export class GraphQueryEngine {
  ...
  run(result: PipelineResult, query: Record<string, string>): QueryResult {
    switch (query.type) {
      case 'violations':    return this.findViolations(result, query.layer);
      case 'dependents':    return this.findDependents(result, query.module);
      case 'dependencies':  return this.findDependencies(result, query.module);
      case 'cycles':        return this.findCycles(result);
      case 'god-modules':   return this.findGodModules(result);
      case 'dead-modules':  return this.findDeadModules(result);
      case 'highly-coupled':return this.findHighlyCoupled(result);
      default:
        return { type: 'unknown', matches: [], summary: `Unknown query type: ${query.type}` };
    }
  }

  private findViolations(result: PipelineResult, layer?: string): QueryResult {
    const smells = layer
      ? result.smells.filter(s => s.module?.toLowerCase().includes(layer.toLowerCase()))
      : result.smells;
    return {
      type:    'violations',
      matches: smells.map(s => ({ module: s.module, type: s.type, severity: s.severity, message: s.message })),
  ...
  private findDependencies(result: PipelineResult, module: string): QueryResult {
    ...
    return {
      ...
      summary: module ? `"${module}" has ${fanOut} outgoing dependencies.` : 'Provide a module name.',
    };
  }
  ...
  private findCycles(result: PipelineResult): QueryResult {
    return {
      type:    'cycles',
      matches: result.cycles.map(c => ({ nodes: c.nodes, length: c.nodes.length })),
  ...
  private findGodModules(result: PipelineResult): QueryResult {
    ...
    return {
      type:    'god-modules',
      matches: matches.map(s => ({ module: s.module, severity: s.severity, fanOut: s.details?.fanOut })),
  ...
  private findHighlyCoupled(result: PipelineResult): QueryResult {
    ...
    return {
      type:    'highly-coupled',
      matches: sorted.map(h => ({ module: h.module, fanOut: h.fanOut, risk: h.risk })),
