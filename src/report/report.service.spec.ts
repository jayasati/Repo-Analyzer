...
const mockResult: Partial<PipelineResult> = {
  ...
  score: {
    overall: 75,
    breakdown: { modularity: 80, coupling: 70, smells: 75 },
  },
  detection: {
    languages: [{ name: 'TypeScript', confidence: 0.9 }],
    framework: 'nestjs',
    analysisDepth: 'framework',
  },
  metrics: {
    moduleCount: 10,
    dependencyCount: 20,
...
describe('ReportService', () => {
  ...
  it('includes score emoji green for score >= 80', () => {
    const highScore = {
      ...
      score: {
        overall: 85,
        breakdown: { modularity: 85, coupling: 85, smells: 85 },
      },
    };
  ...
  it('includes score emoji red for score < 60', () => {
    const lowScore = {
      ...
      score: {
        overall: 40,
        breakdown: { modularity: 40, coupling: 40, smells: 40 },
      },
    };
