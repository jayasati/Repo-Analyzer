...
import {
  GateThresholds, GateContext, GateViolation,
  GateResult, GateMetric, GateSeverity,
} from './gate.types';
...
export class GateService {
  ...
  evaluate(
    ...
    if (current.overallScore < cfg.minScore) {
      violations.push({
        ...
        suggestion: 'Reduce coupling and fix high-severity smells to raise the score.',
      });
    }
    ...
    if (current.cycleCount > cfg.maxCycles) {
      violations.push({
        ...
        suggestion: 'Break cycles by extracting shared abstractions into a common module.',
      });
    }
    ...
    if (current.smellCount > cfg.maxSmells) {
      violations.push({
        ...
        suggestion: 'Prioritise high-severity smells first (god-module, package-tangle).',
      });
    }
    ...
    if (current.avgFanOut > cfg.maxAvgFanOut) {
      violations.push({
        ...
        suggestion: 'Apply Dependency Inversion: depend on abstractions, not concrete modules.',
      });
    }
    ...
    if (previous) {
      ...
      if (drop > cfg.maxScoreDrop) {
        violations.push({
          ...
          delta:     -drop,
          suggestion: 'Review recent changes — a large score drop indicates new smells or cycles.',
        });
      }
      ...
      if (cfg.blockNewCriticalSmells) {
        ...
        if (newCritical.length > 0) {
          violations.push({
            ...
            suggestion: 'These smells were not present in the baseline. Revert or refactor before merging.',
          });
        }
      }
      ...
      if (current.cycleCount > previous.cycleCount) {
        violations.push({
          ...
          suggestion: 'A new cycle was introduced. Check recent module imports for bidirectional dependencies.',
        });
      }
    }
    ...
    const grade       = this.computeGrade(current.overallScore, hasErrors, hasWarnings);
  ...
  private buildMetrics(
    ...
    return [
      metric('Overall Score',   current.overallScore,    previous?.overallScore,    current.overallScore >= DEFAULTS.minScore),
      metric('Coupling Score',  current.couplingScore,   previous?.couplingScore,   current.couplingScore >= DEFAULTS.minCouplingScore),
      metric('Modularity Score',current.modularityScore, previous?.modularityScore, current.modularityScore >= 50),
      metric('Cycles',          current.cycleCount,      previous?.cycleCount,      current.cycleCount === 0),
      metric('Smells',          current.smellCount,      previous?.smellCount,      current.smellCount <= DEFAULTS.maxSmells, current.smellCount <= 10),
      metric('Avg Fan-Out',     Number(current.avgFanOut.toFixed(2)), previous ? Number(previous.avgFanOut.toFixed(2)) : undefined, current.avgFanOut <= DEFAULTS.maxAvgFanOut),
    ];
  }
  ...
  private buildReportBlock(passed: boolean, violations: GateViolation[]): string {
    if (passed && violations.length === 0) {
      return `note "✅ All architecture gates passed" as GateNote #lightgreen\n`;
    }
    const lines = violations
      .map(v => `  ${v.severity === 'error' ? '❌' : '⚠️'} ${v.rule}: ${v.message}`)
      .join('\\n');
