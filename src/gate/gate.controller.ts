...
export class GateController {
  ...
  @ApiOperation({ summary: 'Evaluate architecture gates for a completed analysis' })
  async evaluate(@Body() dto: EvaluateGateDto): Promise<GateResult> {
    const current = await this.cache.get(dto.jobId);
    if (!current) throw new NotFoundException(`Job ${dto.jobId} not found or not yet complete`);

    let previous: PipelineResult | null = null;
    if (dto.baselineJobId) {
      previous = await this.cache.get(dto.baselineJobId);
      if (!previous) throw new NotFoundException(`Baseline job ${dto.baselineJobId} not found`);
    }
