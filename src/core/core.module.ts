...
import { PIPELINE_SCANNERS, PIPELINE_ANALYZERS, PIPELINE_RENDERERS } from './pipeline/pipeline.tokens';
...
@Module({
  ...
  providers: [
    ...
    // ── Scan group ─────────────────────────────────────────────────────────
    {
      ...
      ) => ({ scanner, detector, structuralAnalyzer, semanticAnalyzer, packageGraph, merger }),
      inject: [
        LocalScannerService,
    ...
    // ── Analysis group ──────────────────────────────────────────────────────
    {
      ...
      useFactory: (
        ...
        cycleDetector, smellDetector, metricsService, scoreService,
        confidenceService, baselineComparator, hotspotDetector,
        impactAnalyzer, healthService, summaryService,
      }),
      inject: [
