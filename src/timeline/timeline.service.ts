...
export interface ModuleDelta {
  ...
  resolved:      string[];
  riskChange:    'improved' | 'degraded' | 'unchanged' | 'appeared' | 'disappeared';
}
...
export class TimelineService {
  ...
  async getTimeline(repoUrl: string, limit = 20): Promise<TimelineReport> {
    ...
    const last    = JSON.parse(records[records.length - 1].fullResult) as PipelineResult;
   ...
   */
  async compareSnapshots(fromId: string, toId: string): Promise<GraphDiff & { summary: string }> {
    ...
    const summary = `Score ${scoreDelta >= 0 ? '+' : ''}${scoreDelta} (${fromRecord.overallScore} → ${toRecord.overallScore}). ` +
  ...
  private buildGraphDiff(before: PipelineResult, after: PipelineResult): GraphDiff {
    // Extract module sets
    ...
    for (const s of before.smells) if (s.module) {
      smellsBefore.set(s.module, [...(smellsBefore.get(s.module) ?? []), s.type]);
    }
    for (const s of after.smells) if (s.module) {
      smellsAfter.set(s.module, [...(smellsAfter.get(s.module) ?? []), s.type]);
    }
    ...
    for (const module of allModules) {
      ...
      if (introduced.length > 0 || resolved.length > 0 ||
          !modsBefore.has(module) || !modsAfter.has(module)) {
        changedModules.push({
          module,
          ...
          introduced,
          resolved,
          riskChange:   this.toRiskChange(before_, after_, modsBefore.has(module), modsAfter.has(module)),
        });
      }
    }

    return { addedModules, removedModules, addedEdges, removedEdges, changedModules };
  }

  private extractEdges(result: PipelineResult): Array<{ from: string; to: string }> {
  ...
  private toRiskChange(
    ...
    existedBefore: boolean, existsAfter: boolean,
  ...
  private computeTrajectory(scores: number[]): 'improving' | 'degrading' | 'stable' {
  ...
  private relativeLabel(date: Date, now: Date): string {
    const days = Math.round((now.getTime() - new Date(date).getTime()) / 86400000);
  ...
  private renderDiffDiagram(
    ...
    const lines = [
      ...
      ...diff.changedModules.map(m => {
        const color = m.riskChange === 'improved'  ? '#A3E635'
          : m.riskChange === 'degraded' ? '#F87171'
          : '#FCD34D';
      ...
      ...diff.removedEdges.map(e => `[${e.from}] --> [${e.to}] #F87171 : removed`),
      '',
      '@enduml',
    ];
    return lines.join('\n');
