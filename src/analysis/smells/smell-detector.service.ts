...
export class SmellDetectorService {
  ...
  detect(
    ...
    return [
      ...
      ...this.detectUnstableAbstractions(
        stats,
        thresholds.instabilityThreshold,
      ),
  ...
  private detectGodModules(
    ...
    return Array.from(fanOut.entries())
      ...
      .map(([module, count]) => ({
        ...
        severity:
          count >= threshold * 1.5 ? ('high' as const) : ('medium' as const),
        module,
    ...
    stats.modules.forEach((module) => {
      ...
      if (instability >= threshold && fanIn >= 3) {
        smells.push({
          ...
          severity:
            instability >= 0.9 ? ('high' as const) : ('medium' as const),
          module,
          details: {
  ...
  private detectDeepHierarchy(
    ...
    allNodes.forEach((node) => {
      ...
      if (d > threshold) {
        smells.push({
          ...
          severity:
            d > threshold * 1.5 ? ('high' as const) : ('medium' as const),
          module: node,
    ...
    smells.sort(
      (a, b) =>
        (b.details!['depth'] as number) - (a.details!['depth'] as number),
    );
    return [smells[0]];
  }
