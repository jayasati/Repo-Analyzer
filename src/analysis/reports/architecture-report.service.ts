...
export class ArchitectureReportService {
  ...
  private generateCLI(report: ArchitectureReport): string {
    ...
    if (report.smells.length === 0) lines.push('None');

    report.smells.forEach((s) => lines.push(`- ${s.message}`));

    lines.push('');

    lines.push('Detected Cycles:');

    if (report.cycles.length === 0) lines.push('None');

    report.cycles.forEach((c) => lines.push(`- ${c.nodes.join(' → ')}`));
    ...
    report.recommendations.forEach((r) => lines.push(`- ${r.message}`));
