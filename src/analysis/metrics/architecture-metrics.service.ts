...
export class ArchitectureMetricsService {
  ...
  compute(
    ...
    const maxPossibleDependencies = moduleCount > 1 ? moduleCount * (moduleCount - 1) : 1;

    const dependencyDensity = moduleCount > 1 ? dependencyCount / maxPossibleDependencies : 0;
