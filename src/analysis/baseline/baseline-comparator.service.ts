...
const FRAMEWORK_BASELINE_KEYWORDS: Readonly<Partial<Record<DetectedFramework, string[]>>> = {
  nestjs:    ['nestjs'],
  nextjs:    ['nextjs', 'next'],
  angular:   ['angular'],
  express:   ['express'],
...
export class BaselineComparatorService {
  ...
  compare(
    ...
    const keywords = framework ? (FRAMEWORK_BASELINE_KEYWORDS[framework] ?? []) : [];

    return BASELINES
      .map(baseline => {
        const metricSimilarity = this.computeMetricSimilarity(metrics, baseline.metrics);
        const bonus = keywords.some(kw => baseline.name.toLowerCase().includes(kw))
  ...
  private computeMetricSimilarity(
    ...
    const averageWeightedDifference = weightedDifferenceSum / (totalWeight || 1);
